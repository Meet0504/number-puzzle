...
export default function App({navigation}) {
  ...
  const setHandler = ()=>{
    let copydata=[]
    for(let i=0 ; i<35 ; i++){
      let value=Math.round(Math.random()*34+1)
      while(copydata.includes(value)){
        value=Math.round(Math.random()*34+1)
      }
      copydata.push(value)
    }
    copydata.push('')
...
const checkwin=(copydata)=>{
  if(copydata[0]===1 && copydata[1]===2 && copydata[2]===3 && copydata[3]===4 && copydata[4]===5 && copydata[5]===6 && copydata[6]===7 && copydata[7]===8 && copydata[8]===9 && copydata[9]===10 && copydata[10]===11
    ...
    && copydata[31]===32 && copydata[32]===33 && copydata[33]===34 && copydata[34]===35 && copydata[35]===''){
      AsyncStorage.setItem('moves6',moves.toString())
      AsyncStorage.setItem('time6',counter.toString())
      AsyncStorage.setItem('lastpuzzle','puzzle6')
      setHandler()
